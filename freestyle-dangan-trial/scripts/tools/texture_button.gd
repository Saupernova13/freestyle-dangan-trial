...
func _on_trial_file_selected(path: String):
	# Pre-validate before we leave the start menu so failures surface here
	# (the start menu can show a toast) instead of producing a blank trial room.
...
func _is_valid_trial(path: String) -> bool:
	if not FileAccess.file_exists(path):
		...
		return false
	# Cheapest validity check: can ZIPReader open it and does it contain trial.json?
