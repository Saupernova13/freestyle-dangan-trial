...
export function setSaveStatus(status) {
  ...
  if (status === 'saving') {
    setHtml(el, `${icon('upload', { size: 14 })}<span>Saving…</span>`);
  } else if (status === 'saved') {
    setHtml(el, `${icon('check', { size: 14 })}<span>All changes saved</span>`);
    ...
    setHtml(el, `${icon('alert', { size: 14 })}<span>Save failed — retry</span>`);
  } else {
    setHtml(el, '');
  }
}
